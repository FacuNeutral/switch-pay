//* @type Component
//* @context Branding
//* @utility Lockup SwitchPay: hexagono "S" + "witch" + "Pay" con flecha decorativa sobre la "y". Reutilizable y derivable.

import { cn } from "@/lib/utils";

interface SwitchPayLogoProps {
   /** Altura total del lockup en pixels (la tipografia escala proporcional). */
   size?: number;
   className?: string;
   /** Color del hexagono "S" y del bloque "Pay". */
   accent?: "primary" | "primary-hover" | "white";
   /** Color del texto "witch". */
   textTone?: "white" | "ink";
   /** Mostrar la flecha decorativa sobre la "y". */
   showArrow?: boolean;
}

export default function SwitchPayLogo({
   size = 32,
   className,
   accent = "primary",
   textTone = "white",
   ...
   const accentClass = accent === "primary" ? "text-primary" : accent === "primary-hover" ? "text-primary-hover" : "text-text-hi";
   const textClass = textTone === "white" ? "text-text-hi" : "text-neutral-ink";

   return (
      <span
         className={cn("px-1 inline-flex items-center ", className)}
         aria-label="SwitchPay"
      >
         {/* Hexagono "S" + flecha decorativa inferior (original del SVG) */}
         <svg
            viewBox="0 0 1141 958"
            width={size}
            height={size}
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className={cn(accentClass, "shrink-0")}
            aria-hidden
         >
            <path
            ...
            <path
               ...
               fill="currentColor"
            />
            {/* Flecha decorativa inferior (original) */}
            <path
         ...
         >
            <span className={textClass}>witch</span>
            <span className={cn("relative", accentClass)}>
               {" "}Pa
               <span className="relative inline-block">
                  y
                  {showArrow && (
                     // Flecha superior original del SVG, copiada exactamente, recortada con su propio viewBox
                     <svg
                        viewBox="920 110 210 315"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
         className={cn(accentClass, "absolute left-[100%] -top-0")}
                        style={{ width: size * 0.19, height: size * 0.28 }}
                        aria-hidden
                     >
                        <path
